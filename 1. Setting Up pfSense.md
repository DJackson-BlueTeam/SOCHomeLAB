...
## pfSense will act as a Sandbox Virtual Security Perimeter that will be isolating any VM that are within the firewall network that will be configured to prevent the internal VM accessing the the Host Machine (your computer/laptop) and gaining outside network connection. 
